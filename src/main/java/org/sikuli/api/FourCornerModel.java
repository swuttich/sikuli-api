...
//		VisualModelFinder.logger.step(ir, "parts");
	...
	static public FourCornerModel learnFrom(IplImage trainingImage){
		FourCornerModel model = new FourCornerModel();
...
//		VisualModelFinder.logger.step(trimmedModelImage, "margin trimmed");
		...
		final int d = 12;
		int w = trainingImage.width();
		int h = trainingImage.height();
		//BufferedImage s = trimmedModelImage;
		model.topLeft = new ModelPart(new Rectangle(0,0,d,d), null);
		model.topRight = new ModelPart(new Rectangle(w-d,0,d,d), null);
		model.bottomRight = new ModelPart(new Rectangle(w-d,h-d,d,d), null);
		model.bottomLeft = new ModelPart(new Rectangle(0,h-d,d,d), null);
